describe("Test options", function () {
    describe("dayOfWeekStart", function () {
        it("Should change default start of week", function (done) {
            ...
            setTimeout(function () {
                ...
                setTimeout(function () {
                    expect(picker2.find("th").eq(0).text()).to.be.not.equal(
                        day
                    );
                    expect(picker2.find("td").eq(0).text()).to.be.not.equal(
                        date
                    );
                    done();
                }, 100);
    ...
    describe("disabledDates and startDate", function () {
        it("Should disable some dates in picker and picker should be open on startDate", function (done) {
            ...
            setTimeout(function () {
                var day = picker.find(
                    'td[data-date="8"][data-month="0"][data-year="1986"]'
                );
                expect(day.hasClass("xdsoft_disabled")).to.be.true;
                var start = picker.find(
                    'td[data-date="5"][data-month="0"][data-year="1986"]'
                );
    ...
    describe("defaultDate", function () {
        it("Should open picker on some date", function (done) {
            ...
            setTimeout(function () {
                ...
                expect(start.hasClass("xdsoft_disabled")).to.be.false;
                expect(
                    start.hasClass("xdsoft_current xdsoft_today")
                ).to.be.true;
                done();
            }, 100);
        });
    ...
    describe("Value", function () {
        it("Should set value to plugin", function (done) {
            var input = $(getInput());
            var picker = input
                .datetimepicker({ value: "2011/04/15 05:03" })
                .trigger("mousedown")
                .data(PICKER);

            setTimeout(function () {
                var start = picker.find(
                    'td[data-date="15"][data-month="3"][data-year="2011"]'
                );
                expect(start.length).to.be.equal(1);
                expect(start.hasClass("xdsoft_disabled")).to.be.false;
                done();
